...
2.  **CPU 从 `_entry` 开始执行**: 所有 CPU 核心（hart）都会在机器模式下，从 [`kernel/entry.S`](source/xv6-riscv/kernel/entry.S:15) 的 `_entry` 标签处开始执行。此时，分页机制是关闭的，虚拟地址直接等于物理地址。
3.  **设置初始栈**: `_entry` 的首要任务是为 C 代码的执行准备一个栈。它读取当前核心的 `mhartid`，计算出该核心在 `stack0` 数组中的专属栈空间，并将栈顶指针 `sp` 指向该区域的末尾（因为 RISC-V 的栈是向下增长的）。
    ...
    _entry:
            ...
            add sp, sp, a0
    ```
4.  **跳转到 `start` 函数**: 栈设置好后，`_entry` 通过 `call` 指令跳转到 C 函数 [`start()`](source/xv6-riscv/kernel/start.c:21)。
5.  **切换到监管者模式**: [`start()`](source/xv6-riscv/kernel/start.c:21) 函数执行一系列只能在机器模式下完成的配置：
...
1.  **主 CPU 初始化**: hart 0 负责执行一系列一次性的初始化工作：
    ...
    *   [`procinit()`](source/xv6-riscv/kernel/proc.c:47): 初始化进程表。
    *   [`trapinit()`](source/xv6-riscv/kernel/trap.c), [`trapinithart()`](source/xv6-riscv/kernel/trap.c): 初始化陷阱处理。
    *   ... 初始化其他各种设备和子系统 ...
    *   **[`userinit()`](source/xv6-riscv/kernel/proc.c:236): 创建第一个用户进程！**
...
1.  **`userinit()` 创建进程**: [`main()`](source/xv6-riscv/kernel/main.c) 调用的 [`userinit()`](source/xv6-riscv/kernel/proc.c:236) 函数负责创建第一个进程，通常称为 `init` 进程。
