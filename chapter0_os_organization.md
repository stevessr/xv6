...
2.  **CPU 从 `_entry` 开始执行**: 所有 CPU 核心（hart）都会在机器模式下，从 [`kernel/entry.S`](xv6-riscv/kernel/entry.S:15) 的 `_entry` 标签处开始执行。此时，分页机制是关闭的，虚拟地址直接等于物理地址。
3.  **设置初始栈**: `_entry` 的首要任务是为 C 代码的执行准备一个栈。它读取当前核心的 `mhartid`，计算出该核心在 `stack0` 数组中的专属栈空间，并将栈顶指针 `sp` 指向该区域的末尾（因为 RISC-V 的栈是向下增长的）。
    ...
    _entry:
            ...
            add sp, sp, a0
    ```
4.  **跳转到 `start` 函数**: 栈设置好后，`_entry` 通过 `call` 指令跳转到 C 函数 [`start()`](xv6-riscv/kernel/start.c:21)。
5.  **切换到监管者模式**: [`start()`](xv6-riscv/kernel/start.c:21) 函数执行一系列只能在机器模式下完成的配置：
...
1.  **主 CPU 初始化**: hart 0 负责执行一系列一次性的初始化工作：
    *   [`consoleinit()`](xv6-riscv/kernel/console.c), [`printfinit()`](xv6-riscv/kernel/printf.c): 初始化控制台和打印函数，这样内核才能输出信息。
    *   [`kinit()`](xv6-riscv/kernel/kalloc.c): 初始化物理内存分配器。
    *   [`kvminit()`](xv6-riscv/kernel/vm.c), [`kvminithart()`](xv6-riscv/kernel/vm.c): 创建并启用内核页表，开启分页机制。
    *   [`procinit()`](xv6-riscv/kernel/proc.c:47): 初始化进程表。
    *   [`trapinit()`](xv6-riscv/kernel/trap.c), [`trapinithart()`](xv6-riscv/kernel/trap.c): 初始化陷阱处理。
    *   ... 初始化其他各种设备和子系统 ...
    *   **[`userinit()`](xv6-riscv/kernel/proc.c:236): 创建第一个用户进程！**
...
1.  **`userinit()` 创建进程**: [`main()`](xv6-riscv/kernel/main.c) 调用的 [`userinit()`](xv6-riscv/kernel/proc.c:236) 函数负责创建第一个进程，通常称为 `init` 进程。
    *   调用 `allocproc()` 分配一个 `proc` 结构体。
    *   调用 `uvmfirst()` 分配一页内存，并将一小段硬编码的二进制程序 `initcode` 复制进去。这段代码来自 [`user/initcode.S`](xv6-riscv/user/initcode.S)。
...
3.  **内核执行 `exec`**: 内核的系统调用处理程序会找到 `exec` 对应的实现 `sys_exec`。`exec` 系统调用会用新的程序（这里是 `/init`，其代码在 [`user/init.c`](xv6-riscv/user/init.c) 中）替换掉当前进程的内存和寄存器，然后返回用户空间。

4.  **`init.c` 运行**: 控制权返回用户空间后，进程开始执行 `/init` 程序的 `main` 函数。[`init.c`](xv6-riscv/user/init.c) 的主要工作是：
    *   确保控制台文件描述符被正确设置。
    *   在一个无限循环中，`fork` 一个子进程。
