...
让我们以内核内存分配器 [`kernel/kalloc.c`](xv6-riscv/kernel/kalloc.c:1) 为例。它维护一个空闲内存页的链表 `kmem.freelist`。`kalloc()` 函数从链表头部取下一个空闲页并返回。
...
自旋锁的实现在 [`kernel/spinlock.c`](xv6-riscv/kernel/spinlock.c:19) 中。
...
**获取锁 `acquire()`**:
[`acquire()`](xv6-riscv/kernel/spinlock.c:32) 函数的核心是原子地测试并设置 `lk->locked` 字段。
...
**释放锁 `release()`**:
[`release()`](xv6-riscv/kernel/spinlock.c:70) 的过程相对简单。
...
**使用场景**:
自旋锁最典型的应用就是保护内核内存分配器，如 [`kernel/kalloc.c`](xv6-riscv/kernel/kalloc.c:1) 中的 `kmem.lock`。分配或释放一个内存页的操作非常快，因此使用自旋锁是高效的。
...
睡眠锁的实现在 [`kernel/sleeplock.c`](xv6-riscv/kernel/sleeplock.c:19)，并且它依赖于 [`kernel/proc.c`](xv6-riscv/kernel/proc.c) 中的 `sleep` 和 `wakeup` 机制。
...
**获取锁 `acquiresleep()`**:
[`acquiresleep()`](xv6-riscv/kernel/sleeplock.c:40) 的逻辑体现了“检查-休眠-再检查”的模式。
...
**释放锁 `releasesleep()`**:
[`releasesleep()`](xv6-riscv/kernel/sleeplock.c:66) 的职责是释放锁并唤醒其他等待者。
...
*   研究 [`kernel/riscv.h`](xv6-riscv/kernel/riscv.h:1) 中的内联汇编，了解如何在 C 代码中使用 `amoswap`。
