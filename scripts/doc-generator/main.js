...
const { sourceDir, targetDir, allowedExtensions, allowedFilenames } = require('./config');
const functionExtractor = require('./function-extractor');
const { processFile } = require('./file-processor');
const sidebarGenerator = require('./sidebar');
...
async function main() {
    try {
        ...
        const functionDefinitions = await functionExtractor.run();
